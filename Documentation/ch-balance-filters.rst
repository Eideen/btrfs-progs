...
moving data from single to RAID1). 
...
options to btrfs balance start. Multiple block type can be spesifed per balance.

A filter has the following structure: ``[target block]type[=params][,type=...]``
